...
import { ModelsTabList } from '../const'
import { ModelsTabItem } from '../type'
...
export default function LeftTabsLayout() {
  const [currentTab, setCurrentTab] = useState<ModelsTabItem>('Tasks')
  const [isAddFilterModalDisplay, setIsAddFilterModalDisplay] = useState<boolean>(false)

  const onTabClick = (tabName: ModelsTabItem) => {
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center justify-between lg:hidden">
        <h3 className="text-base font-semibold">Edit Models filters</h3>
        <button className="text-xl" type="button">
          <ExitSVG />
        </button>
      </div>
      <ul className="flex gap-1 text-sm flex-wrap mt-1.5 mb-5">
        {ModelsTabList.map((tabName, index) => (
          <li key={index}>
            <button
              className={classNames(
